/**
 * 
 */
package utils;

/**
 * @author Phong
 *
 */
public final class Constant {
	public static final String SOUND_OF_DOG = "Woof, woof";
	public static final String SOUND_OF_CAT = "Meow";
	public static final String SOUND_OF_ROOSTER = "Cock-a-doodle-doo";
	public static final String SOUND_OF_DUCK = "Quack, quack”";
	public static final String SOUND_OF_PHONE = "Ring Ring";
	public static final String SOUND_OF_FROG = "Croak";
	public static final String ANIMAL_WALKS = "I am walking";
	public static final String ANIMAL_SAYS = "Say Something!";
	public static final String FLY = "I am flying";
	public static final String SING = "I am singing";
	public static final String CHICKEN_SAYS = "Cluck, cluck";
	public static final String DUCK_SAYS = "Quack, quack";
	public static final String SWIMS = "I am swimming";
	public static final String ROOSTER_SAYS = "Cock-a-doodle-doo";
	public static final String MAKE_JOKE = "I make joke";
	public static final String EAT_OTHER_FISH = "I eat other fishes";
	public static final String COLOUR_GREY = "grey";
	public static final String COLOUR_ORANGE = "orange";
	public static final String BODYTYPE_LARGE = "large";
	public static final String BODYTYPE_SMALL = "small";
	public static final String ABILITY_FLY = "fly";
	public static final String ABILITY_SWIM = "swim";
	public static final String ABILITY_WALK = "walk";
	public static final String ABILITY_SING = "sing";
}
